...
func SetupRouter(
	...
	api.Use(middleware.AuthMiddleware(tokens, gatewaySharedSecret))
	{
		...
		api.POST("/customer/profile/photo", h.UpdateCustomerProfilePhoto)
		...
		api.POST("/worker/profile/photo", h.UpdateWorkerProfilePhoto)
		api.POST("/worker/identity-document", h.UploadWorkerIdentityDocument)
		api.POST("/worker/skills", h.AddWorkerSkill)
		api.POST("/worker/skill-upgrades", h.RequestWorkerSkillUpgrade)
		api.POST("/admin/assign-identity-document", h.AssignWorkerIdentityDocument)
		api.POST("/admin/verify-identity-document", h.VerifyWorkerIdentityDocument)
		api.POST("/admin/reject-identity-document", h.RejectWorkerIdentityDocument)
		api.POST("/admin/verify-skill", h.VerifyWorkerSkill)
		api.POST("/admin/reject-skill", h.RejectWorkerSkill)
		api.POST("/admin/verify-skill-upgrade", h.VerifyWorkerSkillUpgrade)
		api.POST("/admin/reject-skill-upgrade", h.RejectWorkerSkillUpgrade)
		...
		api.PATCH("/admin/reports/:report_id/assign", h.AssignReport)
		api.POST("/admin/reports/:report_id/penalty", h.ApplyReportPenalty)
		api.PATCH("/admin/penalties/:penalty_id/cancel", h.CancelPenalty)
		...
		api.PATCH("/payment-method/:id/select", h.SelectPaymentMethod)
		...
		api.GET("/admin/users/:id/profile", h.AdminUserProfile)
